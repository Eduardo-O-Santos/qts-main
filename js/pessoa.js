...
function deleta(id) {
    ...
    find(`http://localhost/qts-main/controlepessoa.php`, options)
        .then(response => {
            response.json()
                .then(data => {
...
const validaRetorno = (result) => {
    ...
    if (result == true) {
        ...
        alerta(0, 'Cadastro realizado com sucesso!', 'Sucesso!', 'listapessoa.php');

    } else {
        $('html, body').animate({ scrollTop: 0 }, 300);
        alerta(1, 'Falha no cadastro!', 'Falha!', 'pessoa.php');
    }
}
